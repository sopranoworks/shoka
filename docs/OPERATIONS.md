...
| `storage.oauth_cleaner.enabled` / `.interval` | bool / duration | no | `true` / `1h` | OAuth dead-series cleaner: deletes token series that are **fully dead** (both access and refresh tokens expired) as soon as they are past their refresh-expiry — there is **no grace** (B-71 Stage 5: expiry ⇒ immediately swept). The OAuth store has no other GC, so dead series otherwise accumulate forever. Only runs when `server.mcp.oauth` is configured; sweeps at boot and then on the interval. Set `enabled:false` to keep dead series visible on the admin page. **Breaking change:** the former `.grace` key was removed; a config still carrying `oauth_cleaner.grace` fails to load (strict key checking). |

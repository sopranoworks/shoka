...
// StartCleaner launches the periodic OAuth dead-series cleaner: a single goroutine
// driven by a time.Ticker, stopped on ctx cancellation. With Enabled=false or
// Interval<=0 it starts NO goroutine AND runs no sweep. Unlike the storage sweep
// workers (StartSnapshotSweep), it runs ONE sweep synchronously at boot BEFORE the
...
func (s *Store) StartCleaner(ctx context.Context, cfg CleanerConfig) {
	...
	// Boot sweep: purge already-dead series immediately, before the first tick is
	// an Interval away. Synchronous so a freshly-started server has GC'd dead series
	// by the time StartCleaner returns (and so restarts always sweep).
	sweep()
	go func() {

...
// three derivative sibling files alongside it: the catalog <proj>.db, the index
// <proj>.index.db, and the deleted-log <proj>.deleted.db (the last added 2026-06-18). They
...
// (namespace, projectName), in a stable order (catalog, index, deleted-log). The files need
// not exist; callers stat/remove/relocate best-effort. Use this everywhere a project's
// sibling DBs are removed or relocated so no op forgets one.

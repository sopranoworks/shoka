...
func TestWSUI_OAuthIssueSelfReturnsToken(t *testing.T) {
	...
	issuer := uiws.OAuthSelfIssuerFunc(func(r *http.Request, _ string, accessTTL time.Duration, _ map[string]any) (string, time.Time, error) {
...
func TestWSUI_OAuthIssueSelfThreadsChosenExpiry(t *testing.T) {
	var gotTTL time.Duration
	issuer := uiws.OAuthSelfIssuerFunc(func(_ *http.Request, _ string, accessTTL time.Duration, _ map[string]any) (string, time.Time, error) {
		gotTTL = accessTTL
...
func TestWSUI_OAuthIssueSelfRefusedForNonAdmin(t *testing.T) {
	var called bool
	issuer := uiws.OAuthSelfIssuerFunc(func(*http.Request, string, time.Duration, map[string]any) (string, time.Time, error) {
		called = true

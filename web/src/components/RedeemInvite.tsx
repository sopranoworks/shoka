...
// code → see the fixed email/scope it grants → set up credentials (password floor +
// optional TOTP; a passkey offered after, if the origin supports it). The email and
// scope come from the invite (the invitee cannot choose them).
        ...
        <form className={styles.card} onSubmit={submit} aria-label="Set up invited account">
          ...
          {useTOTP && totp && (
            <div className={styles.totpBox}>
              ...
              <div className={styles.field}>
                <label htmlFor="rd-code2">6-digit code</label>
                <input id="rd-code2" inputMode="numeric" value={totpCode} onChange={(e) => setTOTPCode(e.target.value)} />
              </div>
            </div>
          )}
          <button className={styles.button} type="submit" disabled={busy}>
